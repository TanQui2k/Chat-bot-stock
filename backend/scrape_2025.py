"""
Scrape ALL Vietnamese stock historical prices for 2025 from HOSE, HNX, UPCOM.
"""
import os
import sys
import time
...
def fetch_history_with_retry(symbol, retries=MAX_RETRIES):
    ...
    for attempt in range(retries):
        try:
            ...
            history = stock.quote.history(start='2025-01-01', end='2025-12-31')
            return history
        ...
        for symbol in symbols:
            ...
            existing_count = session.execute(
                select(func.count(DailyPrice.id)).where(
                    ...
                    DailyPrice.date >= '2025-01-01',
                    DailyPrice.date <= '2025-12-31'
                )
            ).scalar()
            
            if existing_count and existing_count > 0:
                # We could skip, but since 2025 is current, we might just insert new ones
                pass
