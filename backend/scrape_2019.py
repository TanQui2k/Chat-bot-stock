"""
Scrape ALL Vietnamese stock historical prices for 2019 from HOSE, HNX, UPCOM.
"""
import os
import sys
import time
...
def fetch_history_with_retry(symbol, retries=MAX_RETRIES):
    ...
    for attempt in range(retries):
        try:
            ...
            history = stock.quote.history(start='2019-01-01', end='2019-12-31')
            return history
        except Exception as e:
            err_str = str(e).lower()
        ...
        for symbol in symbols:
            ...
            existing_count = session.execute(
                select(func.count(DailyPrice.id)).where(
                    ...
                    DailyPrice.date >= '2019-01-01',
                    DailyPrice.date <= '2019-12-31'
                )
            ).scalar()
